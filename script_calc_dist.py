...
def distance(batch_fn, dummy_matrix, dummy_fns, save_dir, process_id, top=50):
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print 'Usage: {0} <h5py_dir> <process_id> <total_processes> [fea_len=1024]'.format(sys.argv[0])
        sys.exit()
    ...
    fea_len = 1024
    if len(sys.argv) >= 5:
        fea_len = int(sys.argv[4])
    
    if h5py_dir[-1] =='/':
        h5py_dir = h5py_dir[0:-1]
    save_dir = h5py_dir + '_dist'
    ...
    for root, sub_dir, fns in os.walk(h5py_dir):
        for fn in fns:
            ...
            if not idx % process_num:
                ...
                full_fn = os.path.join(root, fn)
                distance(full_fn, dummy_fea, dummy_fns, save_dir, process_id)
